...
define("vs/workbench/electron-browser/workbench.main.nls.zh-cn", {
	...
	"vs/platform/theme/common/colorRegistry": [
		...
		"工作台所选文本的背景颜色（例如输入字段或文本区域）。注意，本设置不适用于编辑器和终端。",
		"文字分隔符的颜色。",
		"文本中链接的前景色。",
		"文本中活动链接的前景色。",
		"预格式化文本段的前景色。",
	...
	"vs/workbench/parts/emmet/electron-browser/emmet.contribution": [
		...
		"不应展开 Emmet 缩写的语言数组。",
		"包含 Emmet 配置文件、代码段和首选项的文件夹路径",
		"试用新版 Emmet 模块（最终会替换旧版单一 Emmet 库）体验所有 Emmet 功能。",
	],
	...
	"vs/workbench/parts/files/browser/files.contribution": [
		...
		"应用程序关闭时将触发热退出。在 Windows/Linux 上关闭最后一个窗口、触发 workbench.action.quit 命令（命令托盘、键绑定、菜单）会引起应用程序关闭。对于任何有文件夹打开的窗口，则不论该窗口是否是最后一个窗口。下次启动时将还原所有未打开文件夹的窗口。若要还原打开有文件夹的窗口，请将“window.reopenFolders”设置为“all”。",
	...
	"vs/workbench/parts/tasks/electron-browser/jsonSchema_v2": [
		...
		"此任务依赖的其他任务。",
		"描述用于执行任务的终端的行为。",
		"控制是否将执行的命令回显到终端。默认关闭。",
		"控制是否显示正在运行任务的终端。默认值为“always”。",
		"定义此任务属于的执行组。如被省略，则此任务不属于任何组。",
	...
	"vs/workbench/services/configuration/node/configurationEditingService": [
		...
		"无法写入配置文件(未知密钥)",
		"无法写入到配置文件(无效的目标)",
		"没有打开任何文件夹，因此无法写入设置。请先打开一个文件夹，然后重试。",
