...
define("vs/workbench/electron-browser/workbench.main.nls.ja", {
	...
	"vs/workbench/parts/debug/electron-browser/debugConfigurationManager": [
		...
		"VS Code が指定されている場合、このコマンドが呼び出され、デバッグ アダプターと引き渡す引数への実行可能パスが確認されます。",
	...
	"vs/workbench/parts/debug/electron-browser/debugService": [
		...
		"複合構成を開始するには、複合に \"構成\" 属性が設定されている必要があります。",
	...
	"vs/workbench/parts/preferences/browser/preferencesEditor": [
		...
		"1 つの設定が一致します",
		"{0} 個の設定が一致します",
		"既定の設定。読み取り専用のテキスト エディター。",
		"編集",
		"設定を置換",
		"設定にコピー",
		"この設定はユーザー設定でなければなりません。ワークスペースのために TypeScript を構成するには、TypeScript ファイルを開き、ステータス バーで TypeScript バージョンをクリックします。",
	...
	"vs/workbench/parts/tasks/electron-browser/task.contribution": [
		...
		"端末の表示",
		"実行中のタスクの終了",
		"タスクは統合ターミナルで実行されます。ターミナルを使用してタスクを管理してください。",
		...
		"エラー: tasks.json ファイルの内容に構文エラーがあります。訂正してからタスクを実行してください。\n",
		"エラー: 指定したタスク設定に検証エラーがあり、使用できません。最初にエラーを修正してください。",
	...
	"vs/workbench/parts/tasks/electron-browser/terminalTaskSystem": [
		"tasks.json にビルド タスクが定義されていません",
		"tasks.json にテスト タスクが定義されていません",
		"タスク \'{0}\' が見つかりません",
	...
	"vs/workbench/parts/tasks/node/processRunnerConfiguration": [
		...
		"タスク {0} はシェル コマンドであり、引数を指定します。コマンド ラインの引用符が正しく解釈されるよう、引数をコマンドに結合してください。",
	],
	...
	"vs/workbench/parts/terminal/electron-browser/terminal.contribution": [
		...
		"ターミナル エミュレーターがシェル プロセスに遅れないようにするためにフロー制御を使用するかどうかを指定します。その主な効果は、コマンドの出力が大量の場合に、^C および他のシグナルの応答がさらに速くなることです。XOFF および XON シグナルの使用を無視するカスタムの ^S または ^Q キー バインドを使用している場合は、この設定を無効にしてください。",
	...
	"vs/workbench/services/themes/electron-browser/themeService": [
		...
		"JSON テーマ ファイルの解析中に問題が発生しました: {0}。\'settings\' は配列ではありません。",
