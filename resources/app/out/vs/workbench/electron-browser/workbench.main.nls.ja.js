...
define("vs/workbench/electron-browser/workbench.main.nls.ja", {
	...
	"vs/workbench/parts/tasks/electron-browser/jsonSchema_v2": [
		...
		"タスクの実行に使用した端末の動作を説明します。",
		"実行されたコマンドがターミナルにエコーされるかどうかを制御します。既定は false です。",
		"タスクを実行しているターミナルを表示するかどうかを制御します。既定は \"always\" です。",
		"このタスクが属する実行グループを定義します。省略した場合、タスクはグループに属しません。",
	...
	"vs/workbench/parts/terminal/electron-browser/terminal.contribution": [
		...
		"ターミナルが Windows で使用するシェルのパス。Windows に付属のシェル (cmd、PowerShell、または Bash on Ubuntu) を使用する場合、64 ビット バージョンを使用するには、C:\\Windows\\System32 ではなく、C:\\Windows\\sysnative を選びます。",
