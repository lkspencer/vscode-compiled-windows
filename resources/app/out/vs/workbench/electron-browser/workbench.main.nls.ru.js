...
define("vs/workbench/electron-browser/workbench.main.nls.ru", {
	...
	"vs/workbench/parts/emmet/electron-browser/emmet.contribution": [
		...
		"Попробуйте новые модули emmet (которые в конечном итоге заменят устаревшую библиотеку emmet), чтобы ознакомиться со всеми функциями emmet.",
	],
	...
	"vs/workbench/parts/extensions/electron-browser/extensionTipsService": [
		"Расширение \'{0}\' не рекомендуется для этого типа файлов.",
	...
	"vs/workbench/parts/files/browser/files.contribution": [
		...
		"Функция \"горячий выход\" будет активирована при закрытии приложения, то есть при закрытии последнего окна в Windows или Linux или при активации команды workbench.action.quit (палитра команд, настраиваемое сочетание клавиш, меню), а также для любых окон с открытыми папками независимо от того, является ли это окно последним. Все окна без открытых папок будут восстановлены при следующем запуске. Чтобы восстановить окна с папками в исходном состоянии перед завершением работы, установите параметр \"window.reopenFolders\" в значение \"all\".",
	...
	"vs/workbench/parts/tasks/electron-browser/jsonSchema_v2": [
		...
		"Описывает поведение терминала, используемого для запуска задачи.",
		"Определяет, стоит ли отправлять выходные данные выполняемой команды в терминал. Значение по умолчанию — false.",
		"Определяет, отображается ли терминал в котором выполняется задача. Значение по умолчанию — \"всегда\".",
		"Определяет, к какой группе выполнения принадлежит эта задача. Если параметр не указан, задача не принадлежит ни к одной из групп.",
		"Определяет, выполняется ли задача в виде процесса или в виде команды оболочки. Значение по умолчанию — \"процесс\".",
		"Номер версии конфигурации.",
		"Пользовательская задача, которая будет настраиваться.",
		"Настройка команд Windows",
		"Настройка команд Mac",
		"Настройка команд Linux",
	],
