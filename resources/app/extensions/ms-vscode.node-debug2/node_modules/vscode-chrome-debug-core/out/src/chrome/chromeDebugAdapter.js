...
class ChromeDebugAdapter {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._exception) {
                const response = {
                    ...
                    breakMode: 'unhandled',
                    details: {
                        stackTrace: this._exception.description && (yield this.sourceMapFormattedException(this._exception.description))
                    }
                };
                return response;
            }
    ...
    onExceptionThrown(params) {
        ...
        this.sourceMapFormattedException(formattedException).then(exceptionStr => {
            this._session.sendEvent(new vscode_debugadapter_1.OutputEvent(exceptionStr, 'stderr'));
        });
    }
    // We parse stack trace from `formattedException`, source map it and return a new string
    sourceMapFormattedException(formattedException) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0, len = exceptionLines.length; i < len; ++i) {
                ...
                const mapped = yield this._sourceMapTransformer.mapToAuthored(clientPath || path, lineNum - 1, columnNum);
                if (mapped && mapped.source && utils.isNumber(mapped.line) && utils.isNumber(mapped.column)) {
                    ...
                    exceptionLines[i] = exceptionLines[i].replace(`${path}:${lineNum}:${columnNum}`, `${mapped.source}:${mapped.line}:${mapped.column}`);
                }
            }
